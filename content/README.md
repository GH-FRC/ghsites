...
- `pages/zh-CN/`: seven retained Simplified Chinese base documents; Stable currently publishes five.
...
- `events/zh-CN/` and `events/en/`: paired, complete event documents for the public activity archive. Every published event requires matching files with body content in both locales. No fictional event is included.
...
Every enabled locale folder must contain exactly the same seven retained page filenames. Competition Results and News remain in the content source but are not published by Stable. Simplified Chinese supplies the complete base. English frontmatter can override individual fields and stable-ID list items; omitted values fall back to Simplified Chinese. An empty English Markdown body uses the complete Simplified Chinese body. Run `npm run check:translations` from the project root to list missing English content without blocking a build.
...
- `pages/zh-CN/`：7 份保留页面的完整简体中文基础文档；Stable 当前发布其中 5 个。
- `pages/en/`：使用相同英文文件名的 7 份独立英文覆盖文档。
- `robots/`：可选的机器人详情文档。公开示例内容不包含虚构的机器人条目。
- `news/`：可选的新闻详情文档。公开示例内容不包含虚构的新闻条目。
- `events/zh-CN/` 与 `events/en/`：用于公开活动归档的完整中英文成对活动文档。每个已发布活动都必须在两种语言中提供同名文件和正文。示例内容不包含虚构活动。
...
每个启用语言文件夹必须包含完全相同的 7 份保留页面文件名。“赛事成果”和“新闻动态”保留在内容源中，但不由 Stable 发布。简体中文提供完整基础内容；英文 Frontmatter 可以覆盖单个字段及带稳定 ID 的列表项目，缺失值回退到简体中文；英文 Markdown 正文为空时使用完整的简体中文正文。请从项目根目录运行 `npm run check:translations`，列出缺失英文内容而不阻断构建。
