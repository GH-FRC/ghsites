...
- `pages/zh-Hant/`: universal Traditional Chinese overlays for the same seven retained pages; Stable currently publishes five.
...
Every enabled locale folder must contain exactly the same seven retained page filenames. Competition Results and News remain in the content source but are not published by Stable. Simplified Chinese supplies the complete base. Traditional Chinese and English frontmatter can override individual fields and stable-ID list items; omitted values fall back to Simplified Chinese. An empty localized Markdown body uses the complete Simplified Chinese body. Run `npm run check:translations` from the project root to report missing English content and require a complete Traditional Chinese release.
...
- `pages/zh-Hant/`：同一组 7 份保留页面的通用繁体中文覆盖文档；Stable 当前发布其中 5 个。
...
每个启用语言文件夹必须包含完全相同的 7 份保留页面文件名。“赛事成果”和“新闻动态”保留在内容源中，但不由 Stable 发布。简体中文提供完整基础内容；繁体中文与英文 Frontmatter 可以覆盖单个字段及带稳定 ID 的列表项目，缺失值回退到简体中文；本地化 Markdown 正文为空时使用完整的简体中文正文。请从项目根目录运行 `npm run check:translations`；英文缺失内容只报告，繁体中文发布内容必须完整。
