...
export const sitePagePlan = [
  ...
  { id: 'sponsors', contentId: 'sponsors', order: 4, showOnHome: true },
  { id: 'contact', contentId: 'contact', order: 5, showOnHome: true },
] as const;
...
export const FIRST_FRC_OFFICIAL_URL =
  ...
  return sitePagePlan.map(({ contentId, id }) => {
    ...
    return {
      href: opensInNewTab
        ...
        : localizedPageHref(locale, contentId),
      label: labels[id],
      isCurrent: !opensInNewTab && context === id,
      opensInNewTab,
    };
  });
}
